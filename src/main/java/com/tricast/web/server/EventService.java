...
public class EventService extends LVSResource {
    ...
    @GET
    @Path("/eventcreationdata")
    @Produces(APPLICATION_JSON)
    public Response getDataForEventCreationScreen() throws SQLException, OutOfTransactionException, IOException {
        log.trace("Requested to get all Event Creation Data");
        try {
            return respondGet(manager.getDataForEvenCreationScreen(workspace));
        } catch (SQLException ex) {
            return respondGet(ex.getMessage(), 500);
        }
    }
    ...
    public Response getAllOpen() throws SQLException, OutOfTransactionException, IOException {
    ...
    @Produces(APPLICATION_JSON)
    @Consumes(APPLICATION_JSON)
    public Response deleteEvent(@PathParam("id") long id) throws SQLException, OutOfTransactionException, IOException {
    ...
    @Produces(APPLICATION_JSON)
    @Consumes(APPLICATION_JSON)
    public Response getById(@PathParam("id") long eventId) throws SQLException, OutOfTransactionException, IOException {
